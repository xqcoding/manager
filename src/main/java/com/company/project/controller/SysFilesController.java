...
public class SysFilesController {
    ...
    public DataResult findListByPage(@RequestBody SysFilesEntity sysFiles) {
        ...
        QueryWrapper<SysFilesEntity> wrapper = new QueryWrapper<>();
        if (StringUtils.isNotBlank(sysFiles.getFileName()) && StringUtils.isNotBlank(sysFiles.getFilePath())) {
            wrapper.and(i -> i.like("file_name",sysFiles.getFileName())
                    .or()
                    .like("file_path",sysFiles.getFilePath())
            );
        } else if (StringUtils.isNotBlank(sysFiles.getFileName())) {
            wrapper.and(i -> i.like("file_name",sysFiles.getFileName())
            );
        } else if (StringUtils.isNotBlank(sysFiles.getFilePath())) {
            wrapper.and(i -> i.like("file_path",sysFiles.getFilePath())
            );
        } else {
        ...
        IPage<SysFilesEntity> iPage = sysFilesService.page(page, wrapper);
