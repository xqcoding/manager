...
-- 2020.7.04添加链接管理 begin
DROP TABLE IF EXISTS `sys_link`;
CREATE TABLE `sys_link` (
  `id` varchar(64) NOT NULL COMMENT '主键',
  `name` varchar(200) DEFAULT NULL COMMENT '链接名称',
  `description` varchar(300) DEFAULT NULL COMMENT '链接描述',
  `url` varchar(100) DEFAULT NULL COMMENT '访问地址URL',
  `short_url` varchar(100) DEFAULT NULL COMMENT '访问短地址URL',
  `total_click_count` tinyint(11) DEFAULT '0' COMMENT '点击次数',
  `is_open` tinyint(2) DEFAULT '0' COMMENT '是否提取码访问(0：否 1：是)',
  `extract_code` varchar(64) DEFAULT NULL COMMENT '提取码',
  `days` tinyint(2) DEFAULT '0' COMMENT '保存天数',
  `expiration_date` datetime DEFAULT NULL COMMENT '过期日期',
  `order_num` int(11) DEFAULT '1000' COMMENT '排序',
  `type` tinyint(4) DEFAULT '1' COMMENT '类型(1:普通;2:短链接;3:其它)',
  `status` tinyint(4) DEFAULT '1' COMMENT '状态(1:正常 0：禁用)',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `deleted` tinyint(4) DEFAULT '1' COMMENT '是否删除(1未删除；0已删除)',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='系统链接管理';
-- 2020.7.04添加链接管理 end